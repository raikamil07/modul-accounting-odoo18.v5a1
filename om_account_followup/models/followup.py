...
class FollowupFollowup(models.Model):
    ...
    class Constraints(models.Model):
        ...
        _sql_constraints = [
            ('company_uniq', 'unique(company_id)', 'Only one follow-up per company is allowed'),
        ]
...
""", )
    ...
    email_template_id = fields.Many2one('mail.template', 'Email Template',
                                        ondelete='set null')
    class Constraints(models.Model):
        _model = 'followup.line'
        _sql_constraints = [
            ('days_uniq', 'unique(followup_id, delay)', 'Days of the follow-up levels must be different'),
        ]
